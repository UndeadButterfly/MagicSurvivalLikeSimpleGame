...
def generate_upgrade_options(bullet_count_upgrades, split_upgrades, bullet_speed_upgrades, player_size_upgrades, bullet_damage_upgrades, bullet_type='PIERCE'):
    pool = [
        {"ids": [3], "text_kor": "[3] 타격 주기 단축 -10%" if bullet_type == 'SATELLITE' else "[3] 공격 간격 -10%", "text_eng": "[3] Attack Interval -10%"},
    ...
    elif bullet_type == 'SATELLITE':
        pool.append({"ids": [1], "text_kor": "[1] 위성 크기 +20%", "text_eng": "[1] Satellite Size +20%"})
    else:
    ...
    if bullet_count_upgrades < 5:
        if bullet_type == 'EXPLOSIVE':
            ...
            pool.append({"ids": [2], "text_kor": "[2] 위성 개수 +1", "text_eng": "[2] Satellite Count +1"})
        else:
            pool.append({"ids": [2], "text_kor": "[2] 관통탄 발사 개수 +2", "text_eng": "[2] Pierce Bullet Count +2"})

    if bullet_type == 'SATELLITE':
        pool.append({"ids": [4], "text_kor": "[4] 회전 궤도 반지름 +15%", "text_eng": "[4] Orbit Radius +15%"})
    else:
        pool.append({"ids": [4], "text_kor": "[4] 발사 방향 +1", "text_eng": "[4] Fire Direction +1"})

    if split_upgrades < 4:
        pool.append({"ids": [11], "text_kor": "[11] 적중 시 분열 +1", "text_eng": "[11] Split on Hit +1"})

    if bullet_speed_upgrades < 3:
        text = "[13] 회전 속도 +30%" if bullet_type == 'SATELLITE' else "[13] 투사체 속도 +30%"
        pool.append({"ids": [13], "text_kor": text, "text_eng": text})

    if player_size_upgrades < 2:
        pool.append({"ids": [14], "text_kor": "[14] 캐릭터 크기 -20%", "text_eng": "[14] Player Size -20%"})

    if bullet_damage_upgrades < 4:
        pool.append({"ids": [15], "text_kor": "[15] 공격력 +1", "text_eng": "[15] Bullet Damage +1"})
